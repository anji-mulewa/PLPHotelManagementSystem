...
//			}
	...
	public boolean deleteHotelInformation(int hotelId) {
		...
		String jpqlDeleteEmployee = "delete from EmployeeInformationBean where hotelId =: hotelId";
		String jpqlDeleteRooms = "delete from RoomInformationBean where hotelId =: hotelId";

		try {

			Query query = entityManager.createQuery(jpql);
			Query queryDeleteEmployee = entityManager.createQuery(jpqlDeleteEmployee);
			Query queryDeleteRoom = entityManager.createQuery(jpqlDeleteRooms);

			query.setParameter("hotelId", hotelId);
			queryDeleteEmployee.setParameter("hotelId", hotelId);
			queryDeleteRoom.setParameter("hotelId", hotelId);

			transaction.begin();
			int result = query.executeUpdate();
			result = queryDeleteEmployee.executeUpdate();
			result = queryDeleteRoom.executeUpdate();
			transaction.commit();
...
""		employeeInformation.setEmail(employeeBean.getEmail());
	...
	public boolean updateEmployeeInformation(EmployeeInformationBean employeeBean) {
		...
		String existingEmail = existingEmployeeInformation.getEmail();
	
		String jpql1 = "from AdminEmployeeUserBean where email = : existingEmail";
		Query query1 = entityManager.createQuery(jpql1);
		query1.setParameter("existingEmail", existingEmail);
		AdminEmployeeUserBean adminEmployeeUserBean = (AdminEmployeeUserBean) query1.getSingleResult();
		boolean isUpdated = false;
