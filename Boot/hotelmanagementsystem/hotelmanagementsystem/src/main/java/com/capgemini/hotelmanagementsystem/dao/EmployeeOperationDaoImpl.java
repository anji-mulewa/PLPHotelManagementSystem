...
public class EmployeeOperationDaoImpl implements EmployeeOperationDao {
	...
	public HotelInformationBean getHotelInformation(AdminEmployeeUserBean adminEmployeeUserBean) {
		...
		try {
			...
			e.printStackTrace();
			// throw new HotelManagementSystemException("Something went wrong... hotel not
			// found");
		}
		return hotelInfo;
	...
	public BookingInformationBean bookUser(BookingInformationBean bookingInformationBean) {
		...
		String jpql = "update RoomInformationBean set roomCount = roomCount-1 where roomId = : roomId ";
		// Query query = entityManager.createQuery(jpql);
		// query.setParameter("roomId", bookingInformationBean.getRoomId());
		// int result = query.executeUpdate();
//		if (hotelValidation.licenseValidation(bookingInformationBean.getLicenceNumber())) {
		try {
			transaction.begin();
